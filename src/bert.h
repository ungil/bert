...
{
    int32_t n_threads;
    int32_t port; // server mode port to bind

    const char* model; // model path
    const char* prompt;
};

//bool bert_params_parse(int argc, char **argv, struct bert_params &params);
...
const char* bert_vocab_id_to_token(struct bert_ctx * ctx, bert_vocab_id id);
